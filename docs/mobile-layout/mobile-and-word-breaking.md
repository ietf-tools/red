...
and `<svg>` are skipped as **non-reflowable content**: material whose alignment carries meaning, so
it keeps its own line breaks and scrolls rather than wraps. Code and artwork are never touched. Why
`<wbr>` rather than a character or a CSS property is in
...
the style the word renders in (monospace in `code`, bold in `strong`, `th`, `dt` and headings) — a
bold word is not excluded the way a bold citation is, because the metric table switches instead. The
...
Citations in a **wider-than-body context** — inside `strong`, `b`, `th`, `dt` or a heading, where
the glyphs are wider than the body metrics describe — are left alone, because marking one on an
underestimated width could hold together a citation that then overflows. Across the surveyed
documents that excluded none, so the guard is precautionary rather than load-bearing.
