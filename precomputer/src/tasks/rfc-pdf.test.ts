...
const getRfcPDFFromTest: typeof fetchRfcPDF = async (rfcNumber: number) => {
  const pdfPath = path.resolve(import.meta.dirname, '..', 'old-rfc-editor.org', 'pdf', `rfc${rfcNumber}.pdf`)
  return fsPromises.readFile(pdfPath, 'base64')
}

test(`rfcBucketPdfToRfcDocument(${RFC_NUMBER_WITH_PDF}, false)`, { timeout: 30_000 }, async () => {
  ...
  const rfcBucketPdfDocument = await rfcBucketPdfToRfcDocument({
    rfcNumber: RFC_NUMBER_WITH_PDF,
    shouldUploadPageImagesToS3: false,
    getRfcCommon,
    getRfcPDF: getRfcPDFFromTest
  })
