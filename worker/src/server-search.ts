...
export async function serverSearch(req: IRequest, _env: Env): Promise<Response | undefined> {
  ...
  try {
    ...
    const html = htmlTemplate`<!DOCTYPE html><html>${safe(head)}<body><h1>Search results for ${JSON.stringify(searchQuery)}</h1><p>${String(totalResults)} result(s) (page ${String(paginationOffset)} of ${String(totalPagesOfResults)})</p><ul>${safe(items.join(''))}</ul><p>${previous}${previous && next ? ' | ' : ''}${next}</p></html>`
