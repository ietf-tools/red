<template>
  <div class="bg-blue-900 dark:bg-blue-950">
    <Heading level="1" style-level="1" class="container mx-auto pl-5 pr-3 py-1 text-white text-balance">
      Search RFCs
    </Heading>
      ...
      <div class="w-full md:w-2/3">
        <ais-search-box
          ...
          :class-names="{
            ...
            'ais-SearchBox-submit': 'bg-blue-200 px-2 flex items-center rounded-r-xs cursor-pointer',
...
const noScriptHtml = computed(() => {
  return `<noscript data-nosnippet>
        ...
        <form method="get" action="${API_NO_JS_SERVER_SEARCH_PATH}" target="${NOSCRIPT_IFRAME_DOM_ID}" class="flex flex-row pt-6 pb-2 md:pb-3" @submit.stop.prevent="handleSearch">
            <input id="search" ref="search-input" placeholder="Find an RFC (number, subseries, title, author, etc.)" v-model="searchQuery" type="search" name="q" autocomplete="search"
