<template>
  <div
    :class="[
      ...
      props.class,
      {
        'screen:[clip-path:polygon(0%_50%,_6%_0%,_96%_0%,100%_50%,96%_100%,6%_100%)]': totalTextLength < 5,
        ...
        'screen:[clip-path:polygon(0%_50%,_4%_0%,_96%_0%,100%_50%,96%_100%,4%_100%)]': totalTextLength > 15,
        'text-base font-semibold': props.size === 'normal',
        'text-xs font-semibold': props.size === 'small'
      }
    ]">
    <template v-if="props.text.length === 1 && props.text[0]">
      <span
        :class="{
    ...
    <template v-else-if="props.text.length === 2 && props.text[0] && props.text[1]">
      <span
        :class="{
...
const totalTextLength = computed(() => {
  return props.text.reduce((acc, item): number => {
    return acc + (typeof item === 'string' ? item.length : getVNodeText(item).length)
  }, 0)
})
</script>
