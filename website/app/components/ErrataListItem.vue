  ...
  <details class="details-open-style -ml-1 open:bg-[rgba(30,30,30,0.07)] dark:open:bg-[rgba(255,255,255,0.1)] rounded">
    ...
    <div class="flex flex-col gap-2 px-2 pt-2 pb-4 mb-4 text-sm">
      <p v-if="props.errataItemForTab.domId">
        <a :href="`#${props.errataItemForTab.domId}`" :class="ANCHOR_COLOR_TAILWIND_STYLE">
...
const orig_text_nodes = computed(() =>
  props.errataItemForTab.orig_text ? preformattedTextToHtml(props.errataItemForTab.orig_text, true) : undefined
)

const correct_text_nodes = computed(() =>
  props.errataItemForTab.correct_text ? preformattedTextToHtml(props.errataItemForTab.correct_text, true) : undefined
)

const notes_nodes = computed(() =>
  props.errataItemForTab.notes ? preformattedTextToHtml(props.errataItemForTab.notes, true) : undefined
)
</script>
